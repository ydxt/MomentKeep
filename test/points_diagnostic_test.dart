  ...
  group('积分诊断测试', () {
    test('诊断并修复用户积分问题', () async {
      ...
      try {
        ...
        // 3. 获取当前积分
        print('\n[步骤 3] 获取当前用户积分...');
        final currentPoints = await dbService.getUserPoints(userId);
        print('当前积分: $currentPoints');
        
        // 4. 获取所有积分明细
        print('\n[步骤 4] 获取所有积分明细...');
        final allBillItems = await dbService.getBillItems(userId);
        ...
        if (allBillItems.isEmpty) {
          ...
          if (currentPoints.round() == calculatedPoints) {
            ...
            await dbService.updatePointsDirectly(userId, calculatedPoints);
            
            // 7. 验证修复
            print('\n[步骤 7] 验证修复结果...');
            final updatedPoints = await dbService.getUserPoints(userId);
