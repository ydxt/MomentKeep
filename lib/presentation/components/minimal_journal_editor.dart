...
import 'package:moment_keep/data/repositories/journal_repository.dart';
...
class _MinimalJournalEditorState extends State<MinimalJournalEditor> {
  ...
  Future<void> _saveJournal() async {
    ...
    if (currentTitle.isNotEmpty) {
      final existingJournals = await JournalRepository().findByTitle(currentTitle);
      if (existingJournals.isNotEmpty) {
        if (mounted) {
          ScaffoldMessenger.of(context).showSnackBar(
            SnackBar(
              content: Text('已存在相同标题的日记：$currentTitle'),
