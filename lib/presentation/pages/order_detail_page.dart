...
import 'package:moment_keep/presentation/pages/my_orders_page.dart' hide LogisticsTrack, LogisticsInfo;
...
class OrderDetailPage extends ConsumerWidget {
      ...
      body: SingleChildScrollView(
        ...
        child: Column(
          ...
          children: [
            ...
            // 订单基础信息
            _buildOrderBasicInfo(context, theme),
            const SizedBox(height: 16),
            // 商品明细
            ...
            const SizedBox(height: 16),
            // 卡密信息（虚拟商品）
            if (order.isElectronic == 1 &&
                (order.status == OrderStatus.completed ||
                    order.status == OrderStatus.pendingReceipt ||
                    order.status == OrderStatus.pendingShipment))
              _buildCardSecretEntry(theme, context),
            const SizedBox(height: 16),
            // 物流摘要卡片（仅待收货状态显示）
            if (order.status == OrderStatus.pendingReceipt) ...[
              _buildLogisticsSummaryCard(theme),
              const SizedBox(height: 16),
            ],
            // 状态轨迹
            ...
            // 操作按钮
            _buildOperationButtons(context, theme, ref),
  ...
  /// 构建订单基础信息
  Widget _buildOrderBasicInfo(BuildContext context, ThemeData theme) {
    return Container(
      padding: const EdgeInsets.all(16),
      decoration: BoxDecoration(
        color: theme.colorScheme.surfaceVariant,
      ...
      child: Column(
        ...
        children: [
          ...
          _buildInfoRow('订单编号', order.id, theme, context: context, copyable: true),
  ...
  Widget _buildInfoRow(String label, String value, ThemeData theme,
      {BuildContext? context, bool copyable = false}) {
    return Padding(
  ...
  Widget _buildCardSecretEntry(ThemeData theme, BuildContext context) {
    return Container(
      padding: const EdgeInsets.all(16),
      decoration: BoxDecoration(
        color: theme.colorScheme.surfaceVariant,
              ...
              InkWell(
                onTap: () {
                  Navigator.push(
                    context,
                    MaterialPageRoute(
                      builder: (_) => CardSecretPage(orderId: order.id),
                    ),
                  );
                },
                child: Row(
  ...
  /// 构建物流摘要卡片
  Widget _buildLogisticsSummaryCard(ThemeData theme) {
    return FutureBuilder<Map<String, dynamic>>(
      future: () async {
        final result = <String, dynamic>{};
        try {
          result['tracks'] = await ProductDatabaseService().getLogisticsTracksByOrderId(order.id);
        } catch (e) {
          debug.log('获取物流轨迹失败: $e');
          result['tracks'] = <LogisticsTrack>[];
        }
        try {
          result['orderData'] = await ProductDatabaseService().getOrderById(order.id);
        } catch (e) {
          debug.log('获取订单数据失败: $e');
      ...
      builder: (context, snapshot) {
        ...
        if (snapshot.hasError) {
          debug.log('物流摘要卡片加载错误: ${snapshot.error}');
          return _buildLogisticsEmptyFallback(theme, null, context);
        }

        final data = snapshot.data;
        final tracks = (data?['tracks'] as List<LogisticsTrack>?) ?? [];
        final orderData = data?['orderData'] as Map<String, dynamic>?;

        if (tracks.isEmpty) {
           return _buildLogisticsEmptyFallback(theme, orderData, context);
         }
        ...
        return Container(
          ...
          child: Column(
            ...
            children: [
              ...
              const SizedBox(height: 12),
              FutureBuilder<LogisticsCompany?>(
                future: firstTrack.logisticsCompanyId != null
                    ? ProductDatabaseService()
                        .getLogisticsCompanyById(firstTrack.logisticsCompanyId!)
                    : Future.value(null),
                builder: (context, companySnapshot) {
                  final companyName =
                      companySnapshot.data?.name ?? '未知物流公司';
                  return Column(
                    crossAxisAlignment: CrossAxisAlignment.start,
                    children: [
                      _buildInfoRow('物流公司', companyName, theme),
                      if (firstTrack.trackingNumber != null &&
                          firstTrack.trackingNumber!.isNotEmpty)
                        _buildInfoRow(
                            '运单号', firstTrack.trackingNumber!, theme,
                            context: context, copyable: true),
                      const SizedBox(height: 8),
                      Row(
                        children: [
                          Icon(
  ...
  Widget _buildLogisticsEmptyFallback(ThemeData theme, Map<String, dynamic>? orderData, BuildContext context) {
    String? trackingNumber;
    String? logisticsCompany;

    if (orderData != null) {
      final logisticsInfoRaw = orderData['logistics_info'];
      if (logisticsInfoRaw != null && logisticsInfoRaw is String && logisticsInfoRaw.isNotEmpty) {
        try {
          final logisticsJson = json.jsonDecode(logisticsInfoRaw) as Map<String, dynamic>;
          final tn = logisticsJson['tracking_number'] as String?;
          final lc = logisticsJson['logistics_company'] as String?;
          ...
          debug.log('解析logistics_info JSON失败: $e');
        }
      }
    }
      ...
      return Container(
        ...
        child: Column(
          ...
          children: [
            ...
            const SizedBox(height: 12),
            _buildInfoRow('物流公司', logisticsCompany ?? '未知物流公司', theme),
            _buildInfoRow('运单号', trackingNumber, theme, context: context, copyable: true),
            const SizedBox(height: 8),
            Row(
              children: [
                Icon(
  ...
  /// 构建操作按钮
  Widget _buildOperationButtons(BuildContext context, ThemeData theme, WidgetRef ref) {
    List<Widget> actions = [];
    ...
    switch (order.status) {
      case OrderStatus.pendingPayment:
        actions = [
          ...
          _buildActionButton('取消订单', () => _cancelOrder(context, ref)),
      ...
      case OrderStatus.pendingReceipt:
        actions = [
          ...
          _buildActionButton('确认收货', () => _confirmReceipt(context, ref),
              isPrimary: true),
        ];
        break;
      case OrderStatus.completed:
            ...
            TextButton(
              onPressed: () async {
                ...
                if (order.pointsUsed > 0 &&
                    ...
                    order.status != OrderStatus.refundAfterSales &&
                  ...
                  try {
                    ...
                    await DatabaseService().updateUserPoints(
                      ...
                      description: '取消订单退款 - ${order.items.isNotEmpty ? order.items.first.name : order.id}',
  ...
  /// 确认收货
  void _confirmReceipt(BuildContext context, WidgetRef ref) {
    showDialog(
      context: context,
      builder: (context) {
        final theme = Theme.of(context);
                ...
                try {
                  ...
                  try {
                    ...
                    if (fundStatus == 'escrow') {
                      final merchantId = orderData?['merchant_id'];
                      final pointsUsedForRelease = (orderData?['points_used'] as num?)?.toDouble() ?? 0;
                      final cashAmountForRelease = (orderData?['cash_amount'] as num?)?.toDouble() ?? 0;
                      final totalAmount = pointsUsedForRelease + cashAmountForRelease;
