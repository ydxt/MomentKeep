...
class _StockRecordsPageState extends ConsumerState<StockRecordsPage> with SingleTickerProviderStateMixin {
  ...
  Future<void> _loadStockRecords() async {
    ...
    try {
      ...
      final productMaps = await databaseService.getAllProducts();
      _products = productMaps.map((map) => StarProduct.fromMap(map)).toList();
    } catch (e) {
  ...
  List<StockRecord> _getFilteredRecords(String type) {
    ...
    if (_searchQuery.isNotEmpty) {
      records = records.where((r) => 
        ...
        _getProductName(r.productId).contains(_searchQuery) ||
                ...
                children: [
                  ...
                  Expanded(
                    child: Column(
                      ...
                      children: [
                        Text(
                          _getProductName(record.productId),
                          style: TextStyle(
        ...
        return AlertDialog(
          ...
          content: SingleChildScrollView(
            child: Column(
              ...
              children: [
                _buildDetailItem('商品', _getProductName(record.productId), theme),
                ...
                TextButton(
                  onPressed: () async {
                    ...
                    try {
                      final databaseService = ProductDatabaseService();
                      final productMaps = await databaseService.getAllProducts();
                      final product = productMaps.where((p) => p['id'] == productId).firstOrNull;
                      if (product == null) {
                        Navigator.pop(context);
                        ScaffoldMessenger.of(context).showSnackBar(
                          SnackBar(
                            content: Text('未找到商品ID: $productId'),
                            backgroundColor: Colors.red,
                            duration: const Duration(seconds: 2),
                          ),
                        );
                      ...
                      if (selectedType == 'in') {
                        ...
                        if (newStock < 0) {
                          ...
                          ScaffoldMessenger.of(context).showSnackBar(
                            SnackBar(
                              content: Text('库存不足，当前库存: $currentStock'),
                              backgroundColor: Colors.red,
                              duration: const Duration(seconds: 2),
                            ),
                          );
                      ...
                      final record = StockRecord(
                        productId: productId,
                        skuId: int.tryParse(skuIdController.text),
                        type: selectedType,
                        quantity: quantity,
                        stockBefore: currentStock,
                        stockAfter: newStock,
                        relatedId: relatedIdController.text.isNotEmpty ? relatedIdController.text : null,
                        remark: remarkController.text.isNotEmpty ? remarkController.text : null,
                        operatorId: 'admin',
                        createdAt: DateTime.now(),
                      );
                      await databaseService.insertStockRecord(record);
                      await _loadStockRecords();
                      if (mounted) {
                        Navigator.pop(context);
                        ScaffoldMessenger.of(context).showSnackBar(
                          SnackBar(
                            content: Text('库存记录添加成功'),
              ...
              onPressed: () async {
                ...
                try {
                  ...
                  if (mounted) {
                    ...
                    ScaffoldMessenger.of(context).showSnackBar(
                      SnackBar(
                        content: Text('补货成功，库存已更新为 $newStock 件'),
                        backgroundColor: theme.colorScheme.primary,
                        duration: const Duration(seconds: 2),
                      ),
                    );
