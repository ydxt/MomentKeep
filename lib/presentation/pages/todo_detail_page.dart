...
class _TodoDetailPageState extends ConsumerState<TodoDetailPage> {
  ...
  String _getCategoryName(String categoryId) {
    final cat = _categories.firstWhere(
      (c) => c.id == categoryId,
      orElse: () => Category(id: '', name: categoryId, type: CategoryType.todo, icon: '', color: 0),
    );
    return cat.name;
  }
  /// 当前完成状态
  ...
  void initState() {
    ...
    _currentCategory = widget.todo.categoryId ?? '';
  ...
  Future<void> _loadInitialCategories() async {
    ...
    if (categoryBloc.state is CategoryLoaded) {
      setState(() {
        _categories = (categoryBloc.state as CategoryLoaded).categories;
      });
    } else {
                ...
                child: GestureDetector(
                  ...
                  child: Row(
                    children: [
                      ...
                      Expanded(
                        child: Text(
                          _getCategoryName(_currentCategory),
                          style: TextStyle(color: theme.colorScheme.onSurfaceVariant),
                        ),
                      ),
                      const SizedBox(width: 4),
