...
class AddTodoPage extends ConsumerStatefulWidget {
  ...
  const AddTodoPage({super.key, this.todo, this.date, this.kanbanColumnId});
...
class _AddTodoPageState extends ConsumerState<AddTodoPage> {
  ...
  Future<void> _saveTodo() async {
    ...
    final isTitleChanged = widget.todo == null ||
        widget.todo!.title.trim().toLowerCase() != currentTitle.toLowerCase();

    List<String> existingTodoIds = [];

    if (isTitleChanged) {
      final todoState = context.read<TodoBloc>().state;
      final existingTodos = todoState is TodoLoaded
          ? todoState.todos.where((t) => t.title.trim().toLowerCase() == currentTitle.toLowerCase()).toList()
          : <Todo>[];
      ...
      if (existingTodos.isNotEmpty) {
        ...
        if (action == 'overwrite') {
          existingTodoIds = existingTodos.map((t) => t.id).toList();
        } else {
  ...
  final newTodo = Todo(
    ...
    tags: _tags.isNotEmpty ? _tags : [_selectedCategory],
        ...
        radius: _locationReminder ? _radius : null,
        locationName: _locationReminder ? _locationName : null,
        kanbanColumnId: widget.kanbanColumnId,
      );
      
      // 通过Bloc添加待办事项
      if (existingTodoIds.isNotEmpty) {
        context.read<TodoBloc>().add(OverwriteTodo(existingTodoIds, newTodo));
      } else {
