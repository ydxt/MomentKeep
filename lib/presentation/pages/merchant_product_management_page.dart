...
class _MerchantProductManagementPageState extends ConsumerState<MerchantProductManagementPage> {
          ...
          actions: [
            ...
            TextButton(
              onPressed: () async {
                Navigator.pop(context);
                try {
                  await _productDatabaseService.deleteProductsByCategoryId(category.id!);
                  await _productDatabaseService.deleteCategory(category.id!);
  ...
  Future<void> _loadData() async {
    try {
      if (widget.merchantId != null) {
        ...
        debugPrint('使用传入的商家ID: $_currentMerchantId, 商家名称: ${widget.merchantName}');
      } else {
      ...
      if (_currentMerchantId != null) {
        final productsResults = await _productDatabaseService.getProductsByMerchantId(_currentMerchantId!);
        products = productsResults.map((map) => StarProduct.fromMap(map)).toList();
        debugPrint('加载商家商品: ${products.length} 个');
      } else {
        final allProductsResults = await _productDatabaseService.getAllProducts();
        products = allProductsResults.map((map) => StarProduct.fromMap(map)).toList();
  ...
  /// 显示库存快捷编辑对话框
  void _showStockEditDialog(StarProduct product) {
          ...
          ElevatedButton(
            onPressed: () async {
              ...
              if (newStock == null || newStock < 0) {
                ScaffoldMessenger.of(context).showSnackBar(
                  const SnackBar(content: Text('请输入有效的库存数量')),
                );
                return;
              }
              Navigator.pop(dialogContext);
              try {
                await _productDatabaseService.updateProductStock(product.id!, newStock);
                await _productDatabaseService.insertStockRecord(StockRecord(
                  productId: product.id!,
                  type: 'adjust',
                  quantity: (newStock - product.stock).abs(),
                  stockBefore: product.stock,
                  stockAfter: newStock,
                  remark: '手动调整',
                  operatorId: _currentMerchantId?.toString() ?? 'unknown',
                  createdAt: DateTime.now(),
                ));
                _loadData();
                if (mounted) {
                  ScaffoldMessenger.of(context).showSnackBar(
                    SnackBar(content: Text('库存已更新为 $newStock')),
                  );
                }
              } catch (e) {
                debugPrint('更新库存失败: $e');
  ...
  void _batchExport() async {
    try {
      ...
      if (allProducts.isEmpty) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(content: Text('暂无商品可导出')),
        );
        return;
      }
      final buffer = StringBuffer();
      buffer.writeln('商品ID,商品名称,分类,价格,库存,状态,类型');
      for (final p in allProducts) {
        ...
        final type = p['product_type'] ?? 'physical';
        buffer.writeln('$id,"$name",$category,$price,$stock,$status,$type');
      }
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text('已导出 ${allProducts.length} 个商品到 ${file.path}'),
          ),
        );
      }
    } catch (e) {
  ...
  /// 显示创建促销活动对话框
  void _showCreatePromotionDialog(StarProduct product) {
    ...
    String promotionType = 'full_reduction';
    DateTime? startTime;
    DateTime? endTime;
      ...
      builder: (context) {
        return StatefulBuilder(
          builder: (context, setDialogState) {
            ...
            return AlertDialog(
              backgroundColor: dialogTheme.colorScheme.surfaceVariant,
              title: Text('创建促销活动', style: TextStyle(color: dialogTheme.colorScheme.onSurface)),
                ...
                child: SizedBox(
                  ...
                  child: Column(
                    ...
                    children: [
                      Text(
                        '商品: ${product.name}',
                        style: TextStyle(color: dialogTheme.colorScheme.onSurfaceVariant, fontSize: 13),
                      ),
                      const SizedBox(height: 16),
                      TextField(
                        ...
                        decoration: InputDecoration(
                          labelText: '活动名称',
                          labelStyle: TextStyle(color: dialogTheme.colorScheme.onSurfaceVariant),
                          border: OutlineInputBorder(
                            borderRadius: BorderRadius.circular(8),
                            borderSide: BorderSide(color: dialogTheme.colorScheme.outline.withOpacity(0.2)),
                          ),
                      ...
                      const SizedBox(height: 16),
                      Text('活动类型', style: TextStyle(color: dialogTheme.colorScheme.onSurface, fontSize: 14)),
                      const SizedBox(height: 8),
                      Row(
                        children: [
                          Expanded(
                            child: GestureDetector(
                              onTap: () => setDialogState(() => promotionType = 'full_reduction'),
                              child: Container(
                                padding: const EdgeInsets.symmetric(vertical: 10),
                                decoration: BoxDecoration(
                                  color: promotionType == 'full_reduction'
                                  ...
                                  border: Border.all(
                                    color: promotionType == 'full_reduction'
                                        ? dialogTheme.colorScheme.primary
                                        : dialogTheme.colorScheme.outline.withOpacity(0.3),
                                  ),
                                ),
                                child: Text(
                                  '满减',
                          ...
                          Expanded(
                            child: GestureDetector(
                              onTap: () => setDialogState(() => promotionType = 'discount'),
                              child: Container(
                                padding: const EdgeInsets.symmetric(vertical: 10),
                                decoration: BoxDecoration(
                                  color: promotionType == 'discount'
                                      ? dialogTheme.colorScheme.primary
                                      : dialogTheme.colorScheme.surface,
                                  borderRadius: BorderRadius.circular(8),
                                  border: Border.all(
                      ...
                      const SizedBox(height: 16),
                      if (promotionType == 'full_reduction') ...[
                        TextField(
                          ...
                          Expanded(
                            child: OutlinedButton(
                                  ...
                                  builder: (context, child) {
                                    return Theme(
                                      data: dialogTheme.copyWith(
                                        colorScheme: dialogTheme.colorScheme,
                                      ),
                                      child: child!,
                                    );
                                  },
                              ...
                              style: OutlinedButton.styleFrom(
                                ...
                                side: BorderSide(color: dialogTheme.colorScheme.outline.withOpacity(0.3)),
                                shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(8)),
                              ),
                              child: Text(
                                startTime != null
                                    ? '开始: ${startTime!.year}-${startTime!.month.toString().padLeft(2, '0')}-${startTime!.day.toString().padLeft(2, '0')}'
                                    : '选择开始时间',
                          ...
                          Expanded(
                            child: OutlinedButton(
                              onPressed: () async {
                                final picked = await showDatePicker(
                                  context: context,
                                  initialDate: endTime ?? (startTime ?? DateTime.now()).add(const Duration(days: 7)),
                              ...
                              child: Text(
                                endTime != null
                                    ? '结束: ${endTime!.year}-${endTime!.month.toString().padLeft(2, '0')}-${endTime!.day.toString().padLeft(2, '0')}'
                                    : '选择结束时间',
                                style: TextStyle(fontSize: 12, color: dialogTheme.colorScheme.onSurface),
                              ),
                            ),
                ...
                ElevatedButton(
                  onPressed: () async {
                    if (nameController.text.trim().isEmpty) {
                      ScaffoldMessenger.of(context).showSnackBar(
                        const SnackBar(content: Text('请输入活动名称'), backgroundColor: Colors.red),
                      );
                      return;
                    }
                    if (promotionType == 'full_reduction') {
                      if (thresholdController.text.trim().isEmpty || discountController.text.trim().isEmpty) {
                        ScaffoldMessenger.of(context).showSnackBar(
                          const SnackBar(content: Text('请输入满减门槛和减免金额'), backgroundColor: Colors.red),
                        );
                        return;
                      }
                    } else {
                      if (discountRateController.text.trim().isEmpty) {
                        ScaffoldMessenger.of(context).showSnackBar(
                          const SnackBar(content: Text('请输入折扣比例'), backgroundColor: Colors.red),
                        );
                        return;
                      }
                    }
                    ...
                    try {
                      ...
                      final promotion = Promotion(
                        ...
                        type: promotionType,
                        thresholdAmount: promotionType == 'full_reduction'
                            ? int.tryParse(thresholdController.text.trim()) ?? 0
                            : (int.tryParse(minSpendController.text.trim()) ?? 0),
                        discountValue: promotionType == 'full_reduction'
                            ? (double.tryParse(discountController.text.trim()) ?? 0)
                            : (double.tryParse(discountRateController.text.trim()) ?? 100),
                        startTime: startTime,
                        endTime: endTime,
                        isActive: true,
                        createdAt: now,
                      ...
                      await _productDatabaseService.insertPromotion(promotion.toMap());
                      Navigator.pop(context);
                      ScaffoldMessenger.of(context).showSnackBar(
                        SnackBar(
                          content: Text('促销活动创建成功'),
                          backgroundColor: dialogTheme.colorScheme.primary,
                        ),
                      );
                    } catch (e) {
                      ...
                      child: Row(
                        ...
                        children: [
                          ...
                          if (product.status == 'active' || product.status == 'approved')
                            IconButton(
                              onPressed: () => _showCreatePromotionDialog(product),
                              icon: Icon(Icons.local_offer, color: theme.colorScheme.tertiary, size: 18),
                              padding: EdgeInsets.zero,
                              constraints: const BoxConstraints(),
                              tooltip: '创建活动',
                            ),
                    ...
                    children: [
                      Row(
                        children: [
                          ...
                          const SizedBox(width: 8),
                          Text(
                            widget.merchantName != null ? '${widget.merchantName} - 商品管理' : '商品管理',
                            style: TextStyle(
                              color: theme.colorScheme.onSurface,
                              fontSize: 20,
                              fontWeight: FontWeight.bold,
