...
import 'star_exchange_page.dart';
...
class _CouponsDetailPageState extends ConsumerState<CouponsDetailPage> with SingleTickerProviderStateMixin {
              ...
              Row(
                ...
                children: [
                  if (coupon['type'] == '折扣券')
                    ...
                    Text(
                      coupon['reward_type'] == 'points'
                          ? '${coupon['amount']}积分'
                          : '¥${((coupon['amount'] is num ? (coupon['amount'] as num).toDouble() : 0.0) / 100).toStringAsFixed(2)}',
                      style: TextStyle(
                        color: isAvailable ? theme.colorScheme.primary : theme.colorScheme.onSurfaceVariant,
                        fontSize: 32,
                        fontWeight: FontWeight.bold,
                    ...
                    child: Text(
                      coupon['condition'] > 0 
                          ? (coupon['reward_type'] == 'points'
                              ...
                              : '满${((coupon['condition'] is num ? (coupon['condition'] as num).toDouble() : 0.0) / 100).toStringAsFixed(0)}可用') 
                          : '无门槛使用',
                      style: TextStyle(
                  ...
                  child: ElevatedButton(
                    onPressed: isAvailable ? () {
                      Navigator.push(
                        context,
                        MaterialPageRoute(
                          builder: (context) => const StarExchangePage(),
                        ),
                      );
                    } : null,
          ...
          child: Column(
            ...
            children: [
              ...
              Row(
                ...
                children: [
                  Text(
                    redPacket['reward_type'] == 'points' ? '积分' : '¥',
                    style: TextStyle(
                      color: isAvailable ? theme.colorScheme.error : theme.colorScheme.onSurfaceVariant,
                      fontSize: 20,
                      fontWeight: FontWeight.bold,
  ...
  // 获取交易记录
  Future<List<Map<String, dynamic>>> _getTransactionHistory(dynamic itemId, String itemName, String type) async {
    ...
    return billItems
        .where((item) {
          if (type == 'coupon') {
            return item.description.contains('优惠券:') && item.description.contains(itemIdStr);
          } else if (type == 'red_packet') {
            return item.description.contains('红包:') && item.description.contains(itemIdStr);
          }
          return false;
        })
        .map((item) => {
