...
class _ProductDetailPageState extends ConsumerState<ProductDetailPage> with SingleTickerProviderStateMixin {
  ...
  // 推荐商品
  List<StarProduct> _similarProducts = [];
  List<StarProduct> _bundleProducts = [];
  ...
  Future<void> _loadRecommendations() async {
    ...
    try {
      ...
      // 并行加载相似商品和搭配购
      final similarResults = await productDb.getSimilarProducts(
        productId: widget.product.id!,
        limit: 6,
      );
      
      final bundleResults = await productDb.getBundleProducts(
        productId: widget.product.id!,
        limit: 4,
      );
      ...
      if (mounted) {
        setState(() {
          _similarProducts = similarResults.map((map) => StarProduct.fromMap(map)).toList();
          _bundleProducts = bundleResults.map((map) => StarProduct.fromMap(map)).toList();
  ...
  Future<void> _recordBrowsingHistory() async {
    try {
      final dbService = DatabaseService();
      final userId = await dbService.getCurrentUserId();
      if (userId == null) return;

      final productDb = ProductDatabaseService();
      await productDb.insertBrowsingHistory({
        'user_id': userId,
        'product_id': widget.product.id!,
        'product_name': widget.product.name,
        'product_image': widget.product.image,
        'product_price': widget.product.price,
        'product_points': widget.product.points,
        'visited_at': DateTime.now().millisecondsSinceEpoch,
      });
    } catch (e) {
      debugPrint('Record browsing history failed: $e');
    }
  }
  ...
  Future<void> _loadActivePromotions() async {
    try {
      ...
      final productDb = ProductDatabaseService();
      final results = await productDb.getActivePromotionsByProductId(productId);
      if (mounted) {
        setState(() {
          _activePromotions = results.map((map) => Promotion.fromMap(map)).toList();
        });
      }
    } catch (e) {
      debugPrint('加载促销活动失败: $e');
    }
  }
  ...
  Future<void> _loadFavoriteState() async {
    try {
      ...
      final productDb = ProductDatabaseService();
      final isFav = await productDb.isFavoriteProduct(userId, productId);
      if (mounted) {
        setState(() {
          _isFavorited = isFav;
        });
                  ...
                  onPressed: () async {
                    ...
                    try {
                      ...
                      if (_isFavorited) {
                        ...
                        await productDb.addFavoriteProduct(userId, productId);
                      }
                      setState(() {
  ...
  Widget _buildProductInfo(StarProduct product, ThemeData theme) {
    ...
    int? discountedPrice;
    int? discountedPoints;
    if (discountPromo != null) {
      discountedPrice = (currentPrice * discountPromo.discountValue / 100).round();
      discountedPoints = (currentPoints * discountPromo.discountValue / 100).round();
    }
          ...
          if (_activePromotions.isNotEmpty)
            Padding(
              ...
              child: Wrap(
                spacing: 8,
                runSpacing: 6,
                children: _activePromotions.map((promo) {
                  String label;
                  if (promo.type == 'full_reduction') {
                    label = '满${promo.thresholdAmount}减${promo.discountValue.toInt()}';
                  } else {
                    final discountPercent = promo.discountValue;
                    label = '${(discountPercent / 10).toStringAsFixed(1)}折';
                  }
                  return Container(
                    padding: const EdgeInsets.symmetric(horizontal: 8, vertical: 4),
                    decoration: BoxDecoration(
  ...
  /// 构建问答内容
  Widget _buildQAContent(StarProduct product, ThemeData theme) {
    return Center(
      child: Column(
        mainAxisAlignment: MainAxisAlignment.center,
        children: [
          ...
          ElevatedButton.icon(
            onPressed: () {
              Navigator.push(
                context,
                MaterialPageRoute(
                  builder: (context) => ProductQAPage(product: product),
                ),
              );
            },
    ...
    return SingleChildScrollView(
      ...
      child: Column(
        ...
        children: [
          ...
          // 空状态
          if (_similarProducts.isEmpty && _bundleProducts.isEmpty)
            Center(
              child: Column(
                mainAxisAlignment: MainAxisAlignment.center,
                children: [
  ...
  Widget _buildRecommendSection({
    required String title,
    required List<StarProduct> products,
    required ThemeData theme,
  }) {
    return Column(
      crossAxisAlignment: CrossAxisAlignment.start,
      children: [
        Padding(
          ...
          child: Row(
            children: [
              Icon(
                title == '相似商品' ? Icons.auto_awesome : Icons.shopping_basket,
                color: theme.colorScheme.primary,
                size: 20,
              ),
              const SizedBox(width: 8),
        ...
        SizedBox(
          ...
          child: ListView.builder(
            ...
            itemBuilder: (context, index) {
              ...
              return _buildRecommendCard(product, theme);
            },
          ),
        ),
      ],
  ...
  Widget _buildRecommendCard(StarProduct product, ThemeData theme) {
    return GestureDetector(
      onTap: () {
        Navigator.push(
          context,
                  ...
                  Expanded(
                    child: ElevatedButton(
                      onPressed: () async {
                        ...
                        final userId = await databaseService.getCurrentUserId() ?? 'default_user';
                        await productDatabaseService.addToCart(product, 1, userId);
                        ScaffoldMessenger.of(context).showSnackBar(
                          SnackBar(
