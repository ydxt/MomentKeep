...
class MerchantOrder {
  ...
  factory MerchantOrder.fromMap(Map<String, dynamic> map) {
    ...
    return MerchantOrder(
      ...
      fundStatus: map['fund_status'] as String?,
    );
  }
}
...
class _MerchantOrderManagementPageState
  ...
  Future<void> _performBatchShip(
    ...
    for (final order in orders) {
      try {
        ...
        try {
          final companies = await _productDatabaseService.getAllLogisticsCompanies(isActive: true);
          final matched = companies.where((c) => c.name == company);
          if (matched.isNotEmpty) {
            companyId = matched.first.id;
          }
        } catch (e) {
          debugPrint('查找物流公司失败: $e');
        }

        await _productDatabaseService.insertLogisticsTrack(star_exchange.LogisticsTrack(
          orderId: order.id,
        ...
        final logisticsData = {
          ...
          'tracks': [
            {'time': _formatTimeString(now), 'location': '发货地', 'description': '商家已发货，等待快递揽收'},
          ],
        };
        final logisticsInfoJson = json.encode(logisticsData);

        await _productDatabaseService.updateOrder(order.id, {
          'status': '已发货',
  ...
  void _rejectOrder(String orderId) {
    ...
    _productDatabaseService.updateOrderStatus(orderId, '已拒单').then((_) async {
      try {
        await _productDatabaseService.restoreOrderStock(orderId, '商家拒单');
      } catch (e) {
        debugPrint('拒单恢复库存失败: $e');
      }

      try {
        final orderData = await _productDatabaseService.getOrderById(orderId);
        ...
        final totalAmount = (orderData?['total_amount'] as num?)?.toDouble() ?? 0;
        ...
        if (fundStatus != null && fundStatus != 'refunded' && userId != null && userId.isNotEmpty) {
          ...
          await _productDatabaseService.updateOrder(orderId, {'fund_status': 'refunded'});
          try {
            await _productDatabaseService.updatePaymentRecordFundStatus(orderId, 'refunded');
          } catch (e) {
  ...
  void _enterLogistics(String orderId) {
    ...
    debugPrint('=== 商家订单管理: _enterLogistics 被调用, orderId=$orderId ===');
    showDialog(
      context: context,
      ...
      debugPrint('=== 商家订单管理: ShippingDialog关闭, result=$result ===');
    });
  }
            ...
            ElevatedButton(
              onPressed: () {
                ...
                _productDatabaseService
                      ...
                      if (orderUserId != null && orderUserId.isNotEmpty) {
                        ...
                        if (pointsUsed != null && pointsUsed.toInt() > 0) {
                          await databaseService.updateUserPoints(
                            orderUserId,
                            pointsUsed.toDouble(),
                            description: '商家取消订单退款 - $productName',
                            transactionType: 'refund',
                            relatedId: orderId,
                          );
                          ref.read(userPointsProvider.notifier).refresh();
                          refundMessage = '已成功取消订单，已退还${pointsUsed.toInt()}积分';
                        }
                        if (cashAmount != null && cashAmount.toDouble() > 0) {
                          debugPrint('商家取消订单现金退款: 订单$orderId, 用户$orderUserId, 现金金额${cashAmount.toDouble()}, 商品:$productName');
                          if (pointsUsed != null && pointsUsed.toInt() > 0) {
                            refundMessage = '已成功取消订单，已退还${pointsUsed.toInt()}积分，现金¥${cashAmount.toDouble()}需线下退款';
                          } else {
                            refundMessage = '已成功取消订单，现金¥${cashAmount.toDouble()}需线下退款';
                          }
                        }
                      }
                    }
                  } catch (e) {
                    debugPrint('取消订单退款处理异常: $e');
                  }
                  ...
                  try {
                    await _productDatabaseService.restoreOrderStock(orderId, '商家取消订单');
                  } catch (e) {
                    debugPrint('恢复库存异常: $e');
                  }
              ...
              Padding(
                ...
                child: Column(
                  ...
                  children: [
                      ...
                      children: [
                        ...
                        Row(
                          ...
                          children: [
                            ...
                            if (order.fundStatus != null) ...[
                              const SizedBox(width: 6),
                              Container(
                                padding: const EdgeInsets.symmetric(horizontal: 8, vertical: 2),
                                decoration: BoxDecoration(
                                  color: order.fundStatus == 'escrow'
                                      ? Colors.orange.withOpacity(0.1)
                                      : order.fundStatus == 'released'
                                          ? Colors.green.withOpacity(0.1)
                                          : Colors.grey.withOpacity(0.1),
                                  borderRadius: BorderRadius.circular(8),
                                ),
                                child: Text(
                                  order.fundStatus == 'escrow'
                                      ? '监管中'
                                      : order.fundStatus == 'released'
                                          ? '已到账'
                                          : '已退款',
                    ...
                    // 物流信息（已发货状态且有物流信息）
                    if (order.status == MerchantOrderStatus.shipped &&
                        order.logisticsInfo != null) ...[
                      const SizedBox(height: 12),
                      _buildLogisticsInfoRow(order.logisticsInfo!, theme),
                    ],
                    // 操作按钮
                    const SizedBox(height: 16),
  ...
  /// 构建物流信息行（运单号 + 复制按钮）
  Widget _buildLogisticsInfoRow(LogisticsInfo logistics, ThemeData theme) {
    return Container(
