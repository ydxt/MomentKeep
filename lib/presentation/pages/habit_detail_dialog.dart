...
class _HabitDetailDialogState extends ConsumerState<HabitDetailDialog> {
  ...
  late int _minCheckInDays;
  late int _checkInPoints;
  ...
  void initState() {
    ...
    _minCheckInDays = widget.habit.minCheckInDays;
    _checkInPoints = widget.habit.checkInPoints;
    _penaltyPoints = widget.habit.penaltyPoints;
                ...
                children: [
                  ...
                  if (_scoringMode == ScoringMode.checkInWithPenalty) ...[
                    ...
                    _buildNumberButtonRow('最小打卡天数', _minCheckInDays, (val) {
                      setState(() { _minCheckInDays = val; _isModified = true; });
                    }, '天', theme, min: 0),
                    const SizedBox(height: 12),
  ...
  Widget _buildNumberButtonRow(String label, int value, Function(int) onChanged, String suffix, ThemeData theme, {int min = 1}) {
    return Row(
      children: [
        ...
        _buildTargetButton('-', () {
          setState(() { if (value > min) onChanged(value - 1); _isModified = true; });
        }, theme),
        const SizedBox(width: 12),
        ...
        const SizedBox(width: 12),
        _buildTargetButton('+', () {
          setState(() { onChanged(value + 1); _isModified = true; });
        }, theme),
      ],
    );
  }
  ...
  Widget _buildScoreRow(ThemeData theme, {String? label}) {
    ...
    return Row(
      children: [
        Text('$displayLabel：', style: TextStyle(color: theme.colorScheme.onSurface, fontSize: 14)),
        _buildTargetButton('-', () {
          setState(() { if (_fullStars > 1) _fullStars--; _isModified = true; });
        }, theme),
        const SizedBox(width: 12),
        ...
        const SizedBox(width: 12),
        _buildTargetButton('+', () {
          setState(() { _fullStars++; _isModified = true; });
        }, theme),
      ],
    );
  }
  ...
  Widget _buildCheckInScoreRow(ThemeData theme) {
    return Row(
      children: [
        Text('打卡得分/天：', style: TextStyle(color: theme.colorScheme.onSurface, fontSize: 14)),
        _buildTargetButton('-', () {
          setState(() { if (_checkInPoints > 1) _checkInPoints--; _isModified = true; });
        }, theme),
        const SizedBox(width: 12),
        ...
        const SizedBox(width: 12),
        _buildTargetButton('+', () {
          setState(() { _checkInPoints++; _isModified = true; });
        }, theme),
      ],
    );
  }
  ...
  Widget _buildPenaltyScoreRow(ThemeData theme) {
    return Row(
      children: [
        Text('不足扣分/天：', style: TextStyle(color: theme.colorScheme.onSurface, fontSize: 14)),
        _buildTargetButton('-', () {
          setState(() { if (_penaltyPoints > 1) _penaltyPoints--; _isModified = true; });
        }, theme),
        const SizedBox(width: 12),
        ...
        const SizedBox(width: 12),
        _buildTargetButton('+', () {
          setState(() { _penaltyPoints++; _isModified = true; });
        }, theme),
      ],
    );
  }
  ...
  Future<void> _pickImageFromGallery() async {
    try {
      ...
      if (pickedFile != null) {
        ...
        setState(() {
          _selectedIconCode = IconHelper.toFileUri(savedPath);
          _isModified = true;
        });
      }
    } catch (e) {
                      ...
                      children: [
                        GridView.builder(
                          ...
                          itemBuilder: (context, index) {
                            final iconData = popularIcons[index];
                            final isSelected = IconHelper.isMaterialIcon(_selectedIconCode) &&
                                int.parse(_selectedIconCode) == iconData.codePoint;
                            return GestureDetector(
                              onTap: () {
                                setState(() {
                            ...
                            children: [
                              ...
                              Wrap(
                                ...
                                children: _commonEmojis.map((emoji) {
                                  final isSelected = _selectedIconCode == emoji;
                                  return GestureDetector(
                                    onTap: () {
                                      setState(() {
                                        _selectedIconCode = emoji;
                                        _isModified = true;
                                      });
                                      Navigator.pop(ctx);
                                    },
                                    child: Container(
                                      ...
                                      height: 48,
                                      decoration: BoxDecoration(
                                        color: isSelected ? t.colorScheme.primary : t.colorScheme.surfaceContainerHighest,
                                        borderRadius: BorderRadius.circular(8),
                                        border: isSelected
                              ...
                              TextField(
                                ...
                                decoration: InputDecoration(
                                  ...
                                  hintStyle: TextStyle(color: t.colorScheme.onSurfaceVariant),
                                  border: OutlineInputBorder(borderRadius: BorderRadius.circular(8)),
                                  contentPadding: const EdgeInsets.symmetric(horizontal: 12, vertical: 10),
                                  suffixIcon: IconButton(
                                    icon: const Icon(Icons.check),
                                    onPressed: () {
                                      final text = customEmojiController.text.trim();
                                      if (text.isNotEmpty) {
                                        setState(() {
                                          _selectedIconCode = text;
                                          _isModified = true;
                                        });
                                        Navigator.pop(ctx);
                                      }
