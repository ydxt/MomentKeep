...
class LogisticsInfo {
  ...
  factory LogisticsInfo.fromMap(Map<String, dynamic> map) {
    ...
    final tracks = tracksList?.map(
      (t) => LogisticsTrack.fromMap(t as Map<String, dynamic>),
    ).toList();
    return LogisticsInfo(
      status: (map['logistics_status'] ?? '已发货') as String,
      description: '物流信息',
...
class Order {
  ...
  factory Order.fromMap(Map<String, dynamic> map) {
    ...
    if (afterSalesRecords == null || afterSalesRecords.isEmpty) {
      if (afterSalesType != null || 
        ...
        final singleRecord = AfterSalesRecord(
          id: ((map['id'] ?? map['order_id']) ?? '') as String,
    ...
    final createdAt = DateTime.fromMillisecondsSinceEpoch((map['created_at'] ?? 0) as int);
    int? remainingPaymentTime;
    if (status == OrderStatus.pendingPayment) {
      final elapsedSeconds = DateTime.now().difference(createdAt).inSeconds;
      remainingPaymentTime = elapsedSeconds < 900 ? 900 - elapsedSeconds : 0;
    }
    ...
    if (map['logistics_info'] != null) {
      try {
        ...
        if (logisticsJson.isNotEmpty) {
          final logisticsMap = json.decode(logisticsJson) as Map<String, dynamic>;
    ...
    return Order(
      ...
      isElectronic: (map['is_electronic'] ?? 0) == 1,
...
class _MyOrdersPageState extends ConsumerState<MyOrdersPage> {
      ...
      setState(() {
        for (var order in _orders) {
          if (order.status == OrderStatus.pendingPayment &&
              ...
              order.remainingPaymentTime! > 0) {
            order.remainingPaymentTime = order.remainingPaymentTime! - 1;
            if (order.remainingPaymentTime == 0) {
  ...
  Future<void> _cancelExpiredOrders() async {
    final expiredOrders = _orders
        .where((o) =>
            o.status == OrderStatus.pendingPayment &&
            o.remainingPaymentTime != null &&
            o.remainingPaymentTime == 0)
        .toList();

    for (final order in expiredOrders) {
      try {
        await ProductDatabaseService().updateOrderStatus(order.id, '已取消');

        final orderData = await ProductDatabaseService().getOrderById(order.id);
        final fundStatus = orderData?['fund_status'] as String?;
        final isPaid = fundStatus != null && fundStatus != 'refunded';

        if (isPaid) {
          try {
            await ProductDatabaseService().restoreOrderStock(order.id, '支付超时自动取消');
          } catch (e) {
            debugPrint('超时订单恢复库存失败: $e');
          }

          try {
            final merchantId = orderData?['merchant_id'];
            if (fundStatus == 'released' && merchantId != null) {
              final merchant = await ProductDatabaseService().getMerchantById(merchantId is int ? merchantId : int.tryParse(merchantId.toString()) ?? 0);
              if (merchant != null) {
                final pointsUsed = orderData?['points_used'] as int? ?? 0;
                if (pointsUsed > 0) {
                  final expireRefundPointsUsed = (orderData?['points_used'] as num?)?.toDouble() ?? 0;
                  final expireRefundCashAmount = (orderData?['cash_amount'] as num?)?.toDouble() ?? 0;
                  final expireRefundAmount = expireRefundPointsUsed + expireRefundCashAmount;
                  await DatabaseService().updateUserPoints(
                    merchant.userId,
                    -expireRefundAmount,
                    description: '超时取消扣回 - ${orderData?['product_name'] ?? '商品'}',
            ...
            debugPrint('超时取消扣回商家收入失败: $e');
          }

          try {
            await ProductDatabaseService().updateOrder(order.id, {'fund_status': 'refunded'});
          } catch (e) {
            debugPrint('超时取消更新fund_status异常: $e');
          }

          if (order.pointsUsed > 0) {
            try {
              final userId = await _databaseService.getCurrentUserId() ?? 'default_user';
              final productName = order.items.isNotEmpty ? order.items.first.name : '商品';
              final storeName = order.storeName;
              await _databaseService.updateUserPoints(
                userId,
                order.pointsUsed.toDouble(),
                description: '超时取消退款 - $productName - $storeName',
                transactionType: 'refund',
                relatedId: order.id,
              );
              ref.read(userPointsProvider.notifier).refresh();
            } catch (e) {
              debugPrint('超时订单退还买家积分失败: $e');
            }
          }
        }
        ...
        try {
          ...
          if (merchantId != null) {
            ...
            final merchant = merchants.where((m) => m.id == merchantId).firstOrNull;
            if (merchant != null) {
              await DatabaseService().database.then((db) async {
                await db.insert('notifications', {
                  ...
                  'title': '订单超时通知',
                  'content': '买家超时未付款，订单已自动关闭 - ${order.items.isNotEmpty ? order.items.first.name : order.id}',
                  'type': 'order',
                  'read': 0,
        ...
        setState(() {
          ...
          if (index != -1) {
            _orders[index] = Order(
              ...
              afterSalesType: order.afterSalesType,
              afterSalesReason: order.afterSalesReason,
              afterSalesDescription: order.afterSalesDescription,
              afterSalesImages: order.afterSalesImages,
              afterSalesCreateTime: order.afterSalesCreateTime,
              afterSalesStatus: order.afterSalesStatus,
              afterSalesRecords: order.afterSalesRecords,
              isElectronic: order.isElectronic,
            );
          }
        });
      } catch (e) {
        debugPrint('取消超时订单失败: $e');
      }
    }
    _applyFilters();
  }
      ...
      for (final order in selectedOrders) {
        for (final item in order.items) {
          try {
            ...
            final productMap = allProducts.where((p) => p['id'] == productId).firstOrNull;
            if (productMap != null) {
              final product = StarProduct.fromMap(productMap);
              await productDb.addToCart(product, item.quantity, userId);
            }
          } catch (e) {
            debugPrint('添加商品到购物车失败: ${item.name} - $e');
          }
        }
      }
  ...
  Widget _buildOrderHeader(Order order, ThemeData theme) {
    ...
    switch (order.status) {
      case OrderStatus.pendingPayment:
        ...
        break;
      case OrderStatus.pendingShipment:
        statusText = order.isElectronic ? '待确认' : '待发货';
        break;
      case OrderStatus.pendingReceipt:
        statusText = '待收货';
    ...
    return Column(
      ...
      children: [
        ...
        Row(
          children: [
            Icon(Icons.confirmation_number_outlined, color: theme.colorScheme.onSurfaceVariant, size: 14),
            const SizedBox(width: 4),
            InkWell(
              onTap: () => _copyToClipboard(order.id, '订单号'),
              child: Text(
                order.id,
                style: TextStyle(
                  color: theme.colorScheme.onSurfaceVariant,
          ...
          Expanded(
            child: Column(
              ...
              children: [
                ...
                if (logistics.trackingNumber != null &&
                    logistics.trackingNumber!.isNotEmpty) ...[
                  const SizedBox(height: 6),
                  GestureDetector(
                    onTap: () {
                      Clipboard.setData(
                          ClipboardData(text: logistics.trackingNumber!));
                      ScaffoldMessenger.of(context).showSnackBar(
                        const SnackBar(
                          content: Text('运单号已复制'),
                ...
                if (logistics.logisticsCompany != null &&
                    logistics.logisticsCompany!.isNotEmpty) ...[
                  const SizedBox(height: 4),
                  Text(
                    '物流公司: ${logistics.logisticsCompany}',
                    style: TextStyle(
                      color: theme.colorScheme.onSurfaceVariant.withOpacity(0.5),
                      fontSize: 10,
                    ),
  ...
  void _payOrder(Order order) {
    ...
    if (order.remainingPaymentTime == null || order.remainingPaymentTime == 0) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(
          content: Text('支付超时，订单已失效，请重新下单'),
          backgroundColor: Colors.red,
        ),
      );
      return;
        ...
        actions: [
          ...
          ElevatedButton(
            onPressed: () async {
              ...
              // 更新订单状态为已支付
              await _databaseService.updateOrderStatus(order.id, order.isElectronic ? '待确认' : '待发货');
              await ProductDatabaseService().updateOrder(order.id, {'fund_status': 'escrow'});
              ...
              switch (order.paymentMethod) {
                case PaymentMethod.points:
                  ...
                  try {
                    final userId = await _databaseService.getCurrentUserId() ?? 'default_user';
                    final orderDataForMerchant = await ProductDatabaseService().getOrderById(order.id);
                    final merchantId = orderDataForMerchant?['merchant_id']?.toString();
                    await ProductDatabaseService().insertPaymentRecord(PaymentRecord(
                      ...
                      merchantId: merchantId != null ? int.tryParse(merchantId) : null,
                      fundStatus: 'escrow',
                    ));
                  } catch (e) {
                    debugPrint('创建积分支付记录失败: $e');
                    if (mounted) {
                      ScaffoldMessenger.of(context).showSnackBar(
                        SnackBar(
                ...
                case PaymentMethod.hybrid:
                  ...
                  try {
                    ...
                    final orderDataForMerchant = await ProductDatabaseService().getOrderById(order.id);
                    final merchantId = orderDataForMerchant?['merchant_id']?.toString();
                    final cashAmount = order.cashAmount.round();
                    await ProductDatabaseService().insertPaymentRecord(PaymentRecord(
                      orderId: order.id,
                      userId: userId,
                      paymentNo: 'PAY${DateTime.now().millisecondsSinceEpoch}${DateTime.now().microsecond}',
                ...
                case PaymentMethod.cash:
                  try {
                    final userId = await _databaseService.getCurrentUserId() ?? 'default_user';
                    final cashAmountInt = order.cashAmount > 0 ? order.cashAmount.round() : (order.totalPrice / 100).round();
                    final orderDataForMerchant = await ProductDatabaseService().getOrderById(order.id);
                    final merchantId = orderDataForMerchant?['merchant_id']?.toString();
                    final paymentRecord = PaymentRecord(
                      ...
                      userId: userId,
                      paymentNo: 'PAY${DateTime.now().millisecondsSinceEpoch}${DateTime.now().microsecond}',
                      amount: cashAmountInt,
                      pointsUsed: 0,
                      cashAmount: cashAmountInt,
                      paymentMethod: 'cash',
                      status: 'success',
                      paidAt: DateTime.now(),
                      createdAt: DateTime.now(),
  ...
  void _cancelOrder(Order order) {
    if (order.isElectronic && order.status != OrderStatus.pendingPayment) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text('虚拟商品支付后不支持取消退款')),
      );
      return;
    }
    final theme = ref.watch(currentThemeProvider);
          ...
          ElevatedButton(
            onPressed: () async {
              ...
              try {
                await ProductDatabaseService().updateOrder(order.id, {'fund_status': 'refunded'});
                try {
                  await ProductDatabaseService().updatePaymentRecordFundStatus(order.id, 'refunded');
                } catch (e) {
              ...
              try {
                await ProductDatabaseService().restoreOrderStock(order.id, '用户取消订单');
              } catch (e) {
                debugPrint('恢复库存异常: $e');
              }
  ...
  void _viewLogistics(Order order) {
    if (order.id.isEmpty) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text('订单信息加载中，请稍候')),
      );
      return;
    }
    Navigator.push(
      context,
      MaterialPageRoute(
        builder: (context) => LogisticsTrackingPage(orderId: order.id),
      ),
    );
  }
          ...
          ElevatedButton(
            onPressed: () async {
              ...
              try {
                ...
                await _databaseService.updateOrderStatus(order.id, '待评价');
                
                if (orderData != null) {
                  final fundStatus = orderData['fund_status'] as String?;
                  if (fundStatus == 'escrow') {
                    final merchantId = orderData['merchant_id'];
                    final pointsUsed = (orderData['points_used'] as num?)?.toDouble() ?? 0;
                    final cashAmount = (orderData['cash_amount'] as num?)?.toDouble() ?? 0;
                    final totalAmount = pointsUsed + cashAmount;
                    if (merchantId != null && totalAmount > 0) {
                      final merchant = await productDatabaseService.getMerchantById(merchantId is int ? merchantId : int.tryParse(merchantId.toString()) ?? 0);
                      if (merchant != null) {
                        await _databaseService.updateUserPoints(
                          merchant.userId,
                          totalAmount,
                          description: '订单收入 - ${orderData['product_name'] ?? '商品'}',
                          transactionType: 'merchant_income',
                          relatedId: order.id,
                        );
                        ref.read(userPointsProvider.notifier).refresh();
                      }
                    }
                    await productDatabaseService.updateOrder(order.id, {'fund_status': 'released'});
                    try {
                      await productDatabaseService.updatePaymentRecordFundStatus(order.id, 'released');
                    } catch (e) {
                      debugPrint('更新PaymentRecord fund_status失败: $e');
                    }
                  }
  ...
  void _reorder(Order order) async {
    ...
    try {
      ...
      // 获取购物车服务和商品服务实例
      final cartService = ProductDatabaseService();
