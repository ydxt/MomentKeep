...
import 'package:moment_keep/presentation/components/shipping_dialog.dart';
...
import 'package:moment_keep/domain/entities/star_exchange.dart' as star_exchange;
...
class _MerchantOrderDetailPageState extends ConsumerState<MerchantOrderDetailPage> {
  ...
  Future<void> _refreshOrderData() async {
    try {
      ...
      if (orderMap != null) {
        ...
        debugPrint('刷新订单: logisticsInfo=${_order.logisticsInfo != null ? "trackingNumber=${_order.logisticsInfo!.trackingNumber}, logisticsCompany=${_order.logisticsInfo!.logisticsCompany}" : "null"}');
        debugPrint('刷新订单: orderMap中logistics_info=${orderMap['logistics_info']}');
      } else {
        debugPrint('刷新订单数据失败: getOrderById返回null for orderId=${_order.id}');
      }
    } catch (e) {
          ...
          children: [
            ...
            Row(
              ...
              children: [
                ...
                InkWell(
                  onTap: () => _copyToClipboard(_order.id, '订单号'),
                  child: Text(
                    _order.id,
                    style: TextStyle(
      ...
      child: Padding(
        ...
        child: Column(
          ...
          children: [
            ...
            const SizedBox(height: 12),
            FutureBuilder<List<star_exchange.LogisticsTrack>>(
              future: ProductDatabaseService().getLogisticsTracksByOrderId(_order.id),
              builder: (context, snapshot) {
                ...
                if (snapshot.hasError) {
                  debugPrint('商家订单物流详情: getLogisticsTracksByOrderId 查询出错，回退到JSON: ${snapshot.error}');
                  return _buildJsonLogisticsContent(theme);
                }

                if (snapshot.hasData && snapshot.data!.isNotEmpty) {
                  final tracks = snapshot.data!;
                  debugPrint('商家订单物流详情: 数据库查到 ${tracks.length} 条物流轨迹');
                  return _buildDbLogisticsContent(theme, tracks);
                }

                debugPrint('商家订单物流详情: 数据库无物流轨迹，回退到JSON');
                return _buildJsonLogisticsContent(theme);
              },
            ),
          ],
  ...
  /// 构建基于数据库的物流内容
  Widget _buildDbLogisticsContent(ThemeData theme, List<star_exchange.LogisticsTrack> tracks) {
    final firstTrack = tracks.first;
    final jsonLogisticsCompany = _order.logisticsInfo?.logisticsCompany;
    final jsonTrackingNumber = _order.logisticsInfo?.trackingNumber;
    final displayTrackingNumber = firstTrack.trackingNumber ?? jsonTrackingNumber ?? '-';
    debugPrint('商家订单物流详情: DB物流单号=${firstTrack.trackingNumber}, JSON物流单号=$jsonTrackingNumber, 物流公司ID=${firstTrack.logisticsCompanyId}, JSON物流公司=$jsonLogisticsCompany');
    return Column(
      crossAxisAlignment: CrossAxisAlignment.start,
      children: [
        _buildInfoRow('物流单号', displayTrackingNumber, theme),
        _buildInfoRow('物流状态', _getLogisticsStatusText(firstTrack.status), theme),
        FutureBuilder<star_exchange.LogisticsCompany?>(
          future: firstTrack.logisticsCompanyId != null
              ? ProductDatabaseService().getLogisticsCompanyById(firstTrack.logisticsCompanyId!)
              : Future.value(null),
          builder: (context, companySnapshot) {
            if (companySnapshot.connectionState == ConnectionState.waiting) {
              return _buildInfoRow('物流公司', '加载中...', theme);
            }
            if (companySnapshot.hasError) {
              return _buildInfoRow('物流公司', jsonLogisticsCompany ?? '未指定', theme);
            }
            final companyName = companySnapshot.data?.name ?? jsonLogisticsCompany ?? '未指定';
            return _buildInfoRow('物流公司', companyName, theme);
          },
        ),
        const SizedBox(height: 12),
        Text(
        ...
        const SizedBox(height: 8),
        ..._buildMiniLogisticsTimeline(theme, tracks),
      ],
    );
  }

  /// 构建基于JSON的物流内容（fallback）
  Widget _buildJsonLogisticsContent(ThemeData theme) {
    final info = _order.logisticsInfo;
    if (info == null) {
      debugPrint('商家订单物流详情(JSON): _order.logisticsInfo 为 null，无物流信息');
      return const Padding(
        padding: EdgeInsets.symmetric(vertical: 16),
        child: Center(
          child: Text(
    ...
    final trackingNumber = info.trackingNumber;
    final logisticsCompany = info.logisticsCompany;
    debugPrint('商家订单物流详情(JSON): 物流单号=$trackingNumber, 物流公司=$logisticsCompany, 物流状态=${info.logisticsStatus}, 轨迹数=${info.tracks?.length ?? 0}');

    if ((trackingNumber == null || trackingNumber.isEmpty) &&
        (logisticsCompany == null || logisticsCompany.isEmpty)) {
      return const Padding(
        padding: EdgeInsets.symmetric(vertical: 16),
        child: Center(
          child: Text(
    ...
    return Column(
      crossAxisAlignment: CrossAxisAlignment.start,
      children: [
        _buildInfoRow('物流单号', trackingNumber ?? '-', theme),
        _buildInfoRow('物流公司', logisticsCompany ?? '-', theme),
        _buildInfoRow('物流状态', info.logisticsStatus ?? '-', theme),
        const SizedBox(height: 12),
        Text(
          '物流轨迹',
          style: TextStyle(
        ...
        const SizedBox(height: 8),
        if (info.tracks != null && info.tracks!.isNotEmpty) ...[
          ...info.tracks!.map((track) {
            final isFirst = info.tracks!.first == track;
            return Padding(
                    ...
                    children: [
                      Container(
                        ...
                        decoration: BoxDecoration(
                          ...
                          borderRadius: BorderRadius.circular(isFirst ? 6 : 4),
                          border: isFirst
                              ? Border.all(
                                  color: theme.colorScheme.surface,
                                  width: 2,
  ...
  /// 构建迷你物流时间线（最新3条）
  List<Widget> _buildMiniLogisticsTimeline(ThemeData theme, List<star_exchange.LogisticsTrack> tracks) {
    final latestTracks = tracks.take(3).toList();
    return latestTracks.asMap().entries.map((entry) {
      final index = entry.key;
      final track = entry.value;
      final isFirst = index == 0;
      final isLast = index == latestTracks.length - 1;
            ...
            Expanded(
              child: Column(
                ...
                children: [
                  ...
                  const SizedBox(height: 4),
                  Text(
                    '${_formatDate(track.trackTime)} ${track.location ?? ''}',
                    style: TextStyle(
                      color: theme.colorScheme.onSurfaceVariant,
                      fontSize: 12,
                    ),
  ...
  /// 获取物流状态文本
  String _getLogisticsStatusText(String status) {
    switch (status) {
      case 'created':
        return '已创建';
      case 'picked':
  ...
  /// 显示物流更新对话框
  void _showLogisticsUpdateDialog() {
    String selectedStatus = 'picked';
                ...
                child: Column(
                  ...
                  children: [
                    DropdownButtonFormField<String>(
                      ...
                      items: const [
                        DropdownMenuItem(value: 'picked', child: Text('已揽收')),
                        DropdownMenuItem(value: 'transporting', child: Text('运输中')),
                        DropdownMenuItem(value: 'delivering', child: Text('派送中')),
                        DropdownMenuItem(value: 'delivered', child: Text('已签收')),
                      ],
                      onChanged: (value) {
                        if (value != null) {
                          setDialogState(() {
  ...
  /// 处理物流更新
  Future<void> _handleLogisticsUpdate(String status, String location, String description) async {
    try {
      final now = DateTime.now();
      final desc = description.isEmpty ? _getLogisticsStatusText(status) : description;

      final productDb = ProductDatabaseService();
      final existingTracks = await productDb.getLogisticsTracksByOrderId(_order.id);

      int? logisticsCompanyId;
      String? trackingNumber;

      if (existingTracks.isNotEmpty) {
        logisticsCompanyId = existingTracks.first.logisticsCompanyId;
        trackingNumber = existingTracks.first.trackingNumber;
      }

      await productDb.insertLogisticsTrack(star_exchange.LogisticsTrack(
        orderId: _order.id,
      ...
      await _syncLogisticsInfoJson(status, location, desc, now);
      ...
      if (mounted) {
        ...
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text('物流状态已更新为"${_getLogisticsStatusText(status)}"'),
            backgroundColor: const Color(0xFF13ec5b),
          ),
        );
      }
    } catch (e) {
      debugPrint('更新物流状态失败: $e');
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text('更新物流状态失败: $e'),
  ...
  /// 同步物流信息到订单的 logistics_info JSON 字段
  Future<void> _syncLogisticsInfoJson(String status, String location, String description, DateTime now) async {
    try {
      final productDb = ProductDatabaseService();
      final orderData = await productDb.getOrderById(_order.id);

      List<dynamic> tracksList = [];
      if (orderData != null && orderData['logistics_info'] != null) {
        try {
          final logisticsJson = orderData['logistics_info'] as String;
          if (logisticsJson.isNotEmpty) {
            final logisticsMap = jsonDecode(logisticsJson) as Map<String, dynamic>;
            if (logisticsMap['tracks'] != null) {
              tracksList = List<dynamic>.from(logisticsMap['tracks']);
            }
          }
        } catch (e) {
          debugPrint('解析已有logistics_info失败: $e');
        }
      }

      tracksList.add({
        'time': _formatDate(now),
        'location': location,
        'description': description,
      });

      final logisticsData = {
        'logistics_status': _getLogisticsStatusText(status),
        'tracks': tracksList,
      };
      ...
      if (orderData != null) {
        try {
          final logisticsJson = orderData['logistics_info'] as String?;
          if (logisticsJson != null && logisticsJson.isNotEmpty) {
            final existingMap = jsonDecode(logisticsJson) as Map<String, dynamic>;
            logisticsData['tracking_number'] = existingMap['tracking_number'];
            logisticsData['logistics_company'] = existingMap['logistics_company'];
          }
        } catch (e) {
          debugPrint('读取logistics_info字段失败: $e');
        }
      }

      await productDb.updateOrder(_order.id, {
        'logistics_info': jsonEncode(logisticsData),
      });
    } catch (e) {
      debugPrint('同步logistics_info JSON失败: $e');
    }
  }
              ...
              onPressed: () {
                ...
                if (noteText.isNotEmpty) {
                  ...
                  final updatedOrder = MerchantOrder(
                    ...
                    isElectronic: _order.isElectronic,
                  );
                  Navigator.pop(dialogContext);
  ...
  void _enterLogistics() {
    debugPrint('=== 商家订单详情: _enterLogistics 被调用, orderId=${_order.id} ===');
    showDialog(
      context: context,
      builder: (ctx) => ShippingDialog(
        orderId: _order.id,
      ...
      debugPrint('=== 商家订单详情: ShippingDialog关闭, result=$result ===');
              ...
              onPressed: () {
                ...
                final updateData = {
                  ...
                  'logistics_info': jsonEncode({
                    ...
                    'tracks': [
                      {'time': nowStr, 'location': '发货地', 'description': '商家已发货，物流公司: $logisticsCompany，运单号: $trackingNumber'},
                    ],
                  }),
                };
                ...
                ProductDatabaseService()
                  ...
                  if (result > 0) {
                    _productDatabaseService.insertLogisticsTrack(star_exchange.LogisticsTrack(
                      orderId: _order.id,
                      trackingNumber: trackingNumber,
                      status: 'created',
                      description: '商家已发货，物流公司: $logisticsCompany，运单号: $trackingNumber',
            ...
            child: ElevatedButton(
              onPressed: () async {
                try {
                  ...
                  final orderData = await _productDatabaseService.getOrderById(_order.id);
                  try {
                    final fundStatus = orderData?['fund_status'] as String?;
                    final merchantId = orderData?['merchant_id'];
                    if (fundStatus == 'released' && merchantId != null) {
                      final merchant = await _productDatabaseService.getMerchantById(merchantId is int ? merchantId : int.tryParse(merchantId.toString()) ?? 0);
                      ...
                      if (merchant != null && pointsUsed > 0) {
                        ...
                        await databaseService.updateUserPoints(
                          merchant.userId,
                          -refundAmount,
                          description: '取消订单扣回 - ${orderData?['product_name'] ?? '商品'}',
                    ...
                    debugPrint('取消订单扣回商家收入失败: $e');
                  }
                  bool hasPointsRefund = false;
                  bool hasCashRefund = false;
                  if (orderData != null) {
                    final orderUserId = orderData['user_id'] as String?;
                    final pointsUsed = orderData['points_used'] as num?;
                    final cashAmount = orderData['cash_amount'] as num?;
                    if (orderUserId != null && orderUserId.isNotEmpty) {
                      if (pointsUsed != null && pointsUsed.toInt() > 0) {
                        await databaseService.updateUserPoints(
                          orderUserId,
                          pointsUsed.toDouble(),
                          description: '商家取消订单退款 - ${_order.productName}',
                          transactionType: 'refund',
                          relatedId: _order.id,
                        );
                        ref.read(userPointsProvider.notifier).refresh();
                        hasPointsRefund = true;
                      }
                      if (cashAmount != null && cashAmount.toDouble() > 0) {
                        await _productDatabaseService.addOperationLog(
                          _order.id,
                          'merchant',
                          'cash_refund',
                          '商家取消订单现金退款 - ${_order.productName}, 金额: $cashAmount',
                        );
                        hasCashRefund = true;
                      }
                    }
                  ...
                  if (context.mounted) {
                    try {
                      await _productDatabaseService.updateOrder(_order.id, {'fund_status': 'refunded'});
                      try {
                        await _productDatabaseService.updatePaymentRecordFundStatus(_order.id, 'refunded');
                      } catch (e) {
                        debugPrint('更新PaymentRecord fund_status失败: $e');
                      }
                    } catch (e) {
                      debugPrint('更新fund_status异常: $e');
                    }
                    try {
                      await _productDatabaseService.restoreOrderStock(_order.id, '商家取消订单');
                    } catch (e) {
                      debugPrint('恢复库存异常: $e');
                    }
                    String refundMessage;
                    if (hasPointsRefund && hasCashRefund) {
                      refundMessage = '订单已取消，积分已退还，现金退款需通过原支付渠道处理';
                    } else if (hasPointsRefund) {
                      refundMessage = '订单已取消，积分已退还给买家';
                    } else if (hasCashRefund) {
                      refundMessage = '订单已取消，现金退款需通过原支付渠道处理';
                    } else {
                      refundMessage = '订单已取消';
                    }
                    ScaffoldMessenger.of(context).showSnackBar(
            ...
            child: ElevatedButton(
              onPressed: () async {
                try {
                  ...
                  try {
                    await _productDatabaseService.restoreOrderStock(_order.id, '商家拒单');
                  } catch (e) {
                    debugPrint('拒单恢复库存失败: $e');
                  }
                  ...
                  try {
                    ...
                    final pointsUsed = orderData?['points_used'] as int? ?? 0;
                    final merchantId = orderData?['merchant_id'];
                    final rejectPointsUsed = (orderData?['points_used'] as num?)?.toDouble() ?? 0;
                    final rejectCashAmount = (orderData?['cash_amount'] as num?)?.toDouble() ?? 0;
                    final totalAmount = rejectPointsUsed + rejectCashAmount;

                    if (fundStatus != null && fundStatus != 'refunded' && userId != null && userId.isNotEmpty) {
                      if (fundStatus == 'released' && merchantId != null) {
                        final merchant = await _productDatabaseService.getMerchantById(
                          merchantId is int ? merchantId : int.tryParse(merchantId.toString()) ?? 0,
                        );
                        if (merchant != null && totalAmount > 0) {
                          await DatabaseService().updateUserPoints(
                            merchant.userId,
                            -totalAmount,
                            description: '拒单扣回 - ${orderData?['product_name'] ?? '商品'}',
                            transactionType: 'merchant_refund',
                            relatedId: _order.id,
                          );
                        }
                      ...
                      if (pointsUsed > 0) {
                        await DatabaseService().updateUserPoints(
                          userId,
                          pointsUsed.toDouble(),
                          description: '拒单退款 - ${orderData?['product_name'] ?? '商品'}',
                          transactionType: 'refund',
                          relatedId: _order.id,
                        );
                        ref.read(userPointsProvider.notifier).refresh();
            ...
            child: ElevatedButton(
              onPressed: () {
                Navigator.push(
                  context,
                  MaterialPageRoute(
                    builder: (context) => LogisticsTrackingPage(orderId: _order.id),
                  ),
                );
              },
              style: ElevatedButton.styleFrom(
  ...
  String _getStatusText(MerchantOrderStatus status) {
    switch (status) {
      ...
      case MerchantOrderStatus.pendingAccept:
        return '待接单';
      case MerchantOrderStatus.pendingShip:
        return _order.isElectronic ? '待确认' : '待发货';
      case MerchantOrderStatus.shipped:
        return '已发货';
      case MerchantOrderStatus.refunding:
        return '退款中';
