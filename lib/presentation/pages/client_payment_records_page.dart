...
class _ClientPaymentRecordsPageState
  ...
  Future<void> _loadPaymentRecords() async {
    ...
    try {
      ...
      final clientRecords = await productDb.getUserPaymentRecords(userId);
  ...
  Widget _buildStatisticsSection(ThemeData theme) {
    ...
    final totalPointsExpense = _paymentRecords
        .where((r) => r.status == 'success')
        .fold(0, (sum, r) => sum + r.pointsUsed);

    final totalCashExpense = _paymentRecords
        .where((r) => r.status == 'success')
          ...
          Row(
            children: [
              ...
              Expanded(
                child: _buildStatCard(
                  '现金支出',
                  '¥${totalCashExpense.toStringAsFixed(2)}',
                  theme.colorScheme.primary,
                  theme,
                ),
              ),
  ...
  Widget _buildPaymentItem(PaymentRecord record, ThemeData theme) {
    String buildDisplayAmount() {
      ...
      if (hasPoints && hasCash) {
        return '${record.pointsUsed}积分 + ¥${record.cashAmount.toStringAsFixed(2)}';
      } else if (hasPoints) {
        return '${record.pointsUsed}积分';
      } else {
        return '¥${record.cashAmount.toStringAsFixed(2)}';
      }
    }
