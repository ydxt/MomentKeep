...
class _HomePageState extends ConsumerState<HomePage> {
  ...
  double _calculateEfficiency() {
    return _calculateEfficiencyForDate(DateTime.now());
  }

  double _calculateYesterdayEfficiency() {
    return _calculateEfficiencyForDate(DateTime.now().subtract(const Duration(days: 1)));
  }
  ...
  Widget _buildStatsOverview(ThemeData theme) {
    ...
    final yesterdayEfficiency = _calculateYesterdayEfficiency();
    final trendDiff = efficiency - yesterdayEfficiency;
    final trendPercent = (trendDiff * 100).round();
    final isUp = trendDiff >= 0;
                        ...
                        Row(
                          children: [
                            Icon(
                              ...
                              color: isUp ? theme.colorScheme.primary : theme.colorScheme.error,
                              size: 16,
                            ),
                            Text(
                              '${trendPercent.abs()}%',
          ...
          Container(
            ...
            decoration: BoxDecoration(
              color: todo.isCompleted
                  ...
                  : PriorityHelper.getColor(todo.priority).withOpacity(0.5),
    ...
    return Stack(
      ...
      children: [
        ...
        Container(
          ...
          child: Center(
            child: iconStr.isEmpty
                ? Icon(Icons.water_drop, color: color, size: 24)
                : IconHelper.buildIconWidget(iconStr, size: 24, color: color),
          ),
        ),
      ],
    );
