...
class _MerchantPaymentRecordsPageState
  ...
  Future<void> _loadPaymentRecords() async {
    ...
    try {
      ...
      final endDate = DateTime(2030, 12, 31, 23, 59, 59, 999);
      final records = await _productDb.getPaymentRecordsByDateRange(
        startDate,
        endDate,
          ...
          content: SingleChildScrollView(
            child: Column(
              ...
              children: [
                ...
                _buildDetailItem(
                  '现金金额',
                  '¥${record.cashAmount.toStringAsFixed(2)}',
                  theme,
                ),
                const SizedBox(height: 12),
                _buildDetailItem(
  ...
  Widget _buildStatisticsSection(ThemeData theme) {
    ...
    final totalPointsIncome = _paymentRecords
        .where((r) => r.status == 'success')
        .fold(0, (sum, r) => sum + r.pointsUsed);

    final totalCashIncome = _paymentRecords
        .where((r) => r.status == 'success')
        .fold(0, (sum, r) => sum + r.cashAmount);
          ...
          Row(
            children: [
              ...
              Expanded(
                child: _buildStatCard(
                  '现金收入',
                  '¥${totalCashIncome.toStringAsFixed(2)}',
  ...
  Widget _buildPaymentItem(PaymentRecord record, ThemeData theme) {
    ...
    String buildDisplayAmount() {
      final prefix = isRefund ? '-' : '+';
      final hasPoints = record.pointsUsed > 0;
      final hasCash = record.cashAmount > 0;
      if (hasPoints && hasCash) {
        return '$prefix${record.pointsUsed}积分 + $prefix¥${record.cashAmount.toStringAsFixed(2)}';
      } else if (hasPoints) {
        return '$prefix${record.pointsUsed}积分';
      } else {
        return '$prefix¥${record.cashAmount.toStringAsFixed(2)}';
      }
    }

    final displayAmount = buildDisplayAmount();
              ...
              children: [
                ...
                Row(
                  ...
                  children: [
                    ...
                    if (record.fundStatus != null) ...[
                      const SizedBox(width: 6),
                      Container(
                        padding: const EdgeInsets.symmetric(horizontal: 8, vertical: 2),
                        decoration: BoxDecoration(
                          color: record.fundStatus == 'escrow'
                              ? Colors.orange.withOpacity(0.1)
                              : record.fundStatus == 'released'
                                  ? Colors.green.withOpacity(0.1)
                                  : Colors.grey.withOpacity(0.1),
                          borderRadius: BorderRadius.circular(8),
                        ),
                        child: Text(
                          record.fundStatus == 'escrow'
                              ? '监管中'
                              : record.fundStatus == 'released'
                                  ? '已到账'
                                  : '已退款',
