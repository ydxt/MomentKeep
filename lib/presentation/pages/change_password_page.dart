    ...
    try {
      ...
      if (securityState is SecurityLoaded) {
        ...
        if (userData.containsKey('buyer_extension') && userData['buyer_extension'] != null) {
          currentPasswordHash = userData['buyer_extension']['password_hash']?.toString();
        } else if (userData.containsKey('seller_extension') && userData['seller_extension'] != null) {
          currentPasswordHash = userData['seller_extension']['password_hash']?.toString();
        } else if (userData.containsKey('admin_extension') && userData['admin_extension'] != null) {
          currentPasswordHash = userData['admin_extension']['password_hash']?.toString();
        } else {
          currentPasswordHash = userData['password_hash']?.toString();
        }

        if (currentPasswordHash == null || currentPasswordHash.isEmpty) {
          _showError('无法获取密码信息，请尝试忘记密码重置');
          return;
        }
