...
class AddProductStepByStepPage extends ConsumerStatefulWidget {
  const AddProductStepByStepPage({super.key, this.product, this.merchantId, required this.onSubmit});
...
class _AddProductStepByStepPageState extends ConsumerState<AddProductStepByStepPage> {
  ...
  Future<void> _loadCurrentMerchantId() async {
    try {
      if (widget.merchantId != null) {
        _currentMerchantId = widget.merchantId;
        debugPrint('添加商品页面 - 使用传入的商家ID: $_currentMerchantId');
        return;
      }
  ...
  Future<void> _loadData() async {
    try {
      final categoryMaps = await _productDatabaseService.getAllCategories();
      final categories = categoryMaps.map((map) => StarCategory.fromMap(map)).toList();
      setState(() {
          ...
          ElevatedButton(
            onPressed: () async {
              if (categoryController.text.isNotEmpty) {
                ...
                // 保存到数据库
                await _productDatabaseService.insertCategory(newCategory.toMap());
  ...
  Future<void> _loadProductData() async {
    ...
    List<StarProductSpec> existingSpecs = product.specs ?? [];
    List<StarProductSku> existingSkus = product.skus ?? [];
    
    if (product.id != null) {
      try {
        final specMaps = await _productDatabaseService.getProductSpecs(product.id!);
        if (specMaps.isNotEmpty) {
          existingSpecs = specMaps.map((m) => StarProductSpec.fromMap(m)).toList();
        }
        final skuMaps = await _productDatabaseService.getProductSkus(product.id!);
        if (skuMaps.isNotEmpty) {
          existingSkus = skuMaps.map((m) => StarProductSku.fromMap(m)).toList();
        }
      } catch (e) {
        debugPrint('从数据库加载规格/SKU失败: $e');
      }
    }
    ...
    setState(() {
      ...
      if (existingSkus.isNotEmpty) {
        ...
        _formData['skus'] = existingSkus.map((sku) => sku.toMap()).toList();
      } else {
        _formData['specType'] = 'single';
      }
      
      if (existingSpecs.isNotEmpty) {
        _formData['specs'] = existingSpecs.map((spec) => spec.toMap()).toList();
      } else {
        _formData['specs'] = [];
      }
  ...
  Future<void> _saveAsDraftToDatabase() async {
    ...
    try {
      ...
      if (widget.product?.id != null) {
        await _productDatabaseService.updateProduct(draftProduct.id!, draftProduct.toMap());
      } else {
        await _productDatabaseService.insertProduct(draftProduct.toMap());
      }
  ...
  Future<void> _submitProduct() async {
    ...
    try {
      ...
      // 保存商品
      if (widget.product?.id != null) {
        await _productDatabaseService.updateProduct(product.id!, product.toMap());
      } else {
        final productId = await _productDatabaseService.insertProduct(product.toMap());
        if (product.id == null && productId > 0) {
          final updatedProduct = StarProduct(
            ...
            skus: product.skus?.map((sku) => StarProductSku(
              id: sku.id,
              productId: productId,
              skuCode: sku.skuCode,
              ...
              price: sku.price,
              points: sku.points,
              hybridPrice: sku.hybridPrice,
              hybridPoints: sku.hybridPoints,
              ...
              supportPointsPayment: sku.supportPointsPayment,
              supportCashPayment: sku.supportCashPayment,
              supportHybridPayment: sku.supportHybridPayment,
            )).toList(),
            specs: product.specs?.map((spec) => StarProductSpec(
              id: spec.id,
              productId: productId,
              name: spec.name,
              values: spec.values,
      ...
      final effectiveProductId = product.id ?? widget.product?.id;
      if (effectiveProductId != null && _formData['specType'] == 'multiple') {
        await _productDatabaseService.deleteProductSpecsByProductId(effectiveProductId);
        await _productDatabaseService.deleteProductSkusByProductId(effectiveProductId);

        final specsList = _formData['specs'] as List;
        for (var specMap in specsList) {
          final map = Map<String, dynamic>.from(specMap);
          final specData = {
            'product_id': effectiveProductId,
            'name': map['name'] ?? '',
            'values': (map['values'] is List) ? (map['values'] as List).join(',') : (map['values'] ?? ''),
            'sort_order': map['sort_order'] ?? 0,
            'created_at': DateTime.now().millisecondsSinceEpoch,
            'updated_at': DateTime.now().millisecondsSinceEpoch,
          };
          await _productDatabaseService.insertProductSpec(specData);
        }

        final skusList = _formData['skus'] as List;
        for (var skuMap in skusList) {
          final map = Map<String, dynamic>.from(skuMap);
          final specValues = map['specValues'] is Map
              ? Map<String, dynamic>.from(map['specValues'])
              : <String, dynamic>{};
          final skuData = {
            'product_id': effectiveProductId,
            'sku_code': map['skuCode'] ?? 'SKU${DateTime.now().millisecondsSinceEpoch}',
            'spec_values': jsonEncode(specValues),
            'price': map['price'] ?? 0,
            'points': map['points'] ?? 0,
            'hybrid_price': map['hybridPrice'] ?? 0,
            'hybrid_points': map['hybridPoints'] ?? 0,
            'cost_price': map['costPrice'] ?? 0,
            'stock': map['stock'] ?? 0,
            'image': map['image'] ?? '',
            ...
            'updated_at': DateTime.now().millisecondsSinceEpoch,
            'is_deleted': 0,
            'support_points_payment': (map['supportPointsPayment'] ?? _formData['supportPointsPayment'] ?? true) ? 1 : 0,
            'support_cash_payment': (map['supportCashPayment'] ?? _formData['supportCashPayment'] ?? true) ? 1 : 0,
            'support_hybrid_payment': (map['supportHybridPayment'] ?? _formData['supportHybridPayment'] ?? true) ? 1 : 0,
          };
          await _productDatabaseService.insertProductSku(skuData);
        }
      }
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text('商品提交成功，等待审核')),
      );
                ...
                Container(
                  ...
                  child: Row(
                    children: [
                      ...
                      IconButton(
                        onPressed: () async {
                          final keyword = _productNameController.text.trim();
                          if (keyword.isEmpty) {
                            ScaffoldMessenger.of(context).showSnackBar(
                              SnackBar(content: Text('请先输入商品名称')),
                            );
                            return;
                          }
                          try {
                            ...
                            final duplicates = allProducts.where((p) =>
                              (p['name'] as String?)?.contains(keyword) == true
                            ).toList();
                            if (!mounted) return;
                            showDialog(
                              context: context,
                              builder: (ctx) => AlertDialog(
                                title: Text('查重结果'),
                                content: Text(duplicates.isEmpty
                                  ? '未发现同名商品，可以继续添加'
                                  : '发现 ${duplicates.length} 个相似商品：\n${duplicates.take(5).map((p) => "· ${p['name']}").join("\n")}'),
                ...
                Container(
                  ...
                  child: Row(
                    children: [
                      ...
                      IconButton(
                        onPressed: () {
                          _productCodeController.selection = TextSelection(baseOffset: 0, extentOffset: _productCodeController.text.length);
                      ...
                      SizedBox(
                        ...
                        child: SingleChildScrollView(
                          ...
                          child: DataTable(
                            ...
                            dataRowColor: MaterialStateProperty.all(theme.colorScheme.surfaceContainerHighest),
                          ...
                          builder: (ctx) => AlertDialog(
                            ...
                            content: SingleChildScrollView(
                              child: Column(
                                ...
                                crossAxisAlignment: CrossAxisAlignment.start,
                                children: [
                                  Text('1. 填写商品名称，可点击搜索按钮查重'),
                                  SizedBox(height: 8),
                                  Text('2. 商品编码为唯一标识，可自动生成或手动输入'),
                                  SizedBox(height: 8),
                                  Text('3. 选择商品分类和类型（实物/虚拟）'),
                                  SizedBox(height: 8),
                                  Text('4. 设置价格、库存和规格信息'),
                                  SizedBox(height: 8),
                                  Text('5. 上传商品图片和详情描述'),
                                  SizedBox(height: 8),
                                  Text('6. 虚拟商品需配置卡密或权益信息'),
                                  SizedBox(height: 8),
                                  Text('7. 确认信息无误后提交审核'),
                                ],
                              ),
