...
class ResumePomodoro extends PomodoroEvent {
  ...
  const ResumePomodoro({
    required this.pomodoroId,
    required this.remainingTime,
    required this.originalTotalTime,
    required this.initialState,
    this.isVibrationEnabled = true,
    this.isAudioEnabled = true,
    this.isRandomAudioEnabled = false,
...
class PomodoroBloc extends Bloc<PomodoroEvent, PomodoroBlocState> {
  ...
  FutureOr<void> _onStartPomodoro(
    ...
    try {
      ...
      final insertId = await db.insert('pomodoro_records', {
      ...
      if (insertId <= 0) {
        print('WARNING: pomodoro insert returned id=$insertId, pomodoro_id=${session.pomodoroId}');
      }
    } catch (e) {
      print('CRITICAL: Error recording pomodoro start: $e, pomodoro_id=${session.pomodoroId}');
    }
  ...
  FutureOr<void> _onResumePomodoro(
      ResumePomodoro event, Emitter<PomodoroBlocState> emit) async {
    _timer?.cancel();
    ...
    final session = PomodoroSession(
      ...
      totalTime: event.originalTotalTime,
      isVibrationEnabled: event.isVibrationEnabled,
      isAudioEnabled: event.isAudioEnabled,
      isRandomAudioEnabled: event.isRandomAudioEnabled,
      isAudioSelectionEnabled: event.isAudioSelectionEnabled,
    ...
    emit(PomodoroRunning(session));
    _startTimer(event.remainingTime);
    ...
    try {
      ...
      final rowsAffected = await db.update(
          ...
          where: 'pomodoro_id = ?',
          whereArgs: [event.pomodoroId]);
      if (rowsAffected == 0) {
        print('WARNING: pomodoro resume update affected 0 rows, pomodoro_id=${event.pomodoroId}');
      }
    } catch (e) {
      print('CRITICAL: Error resuming pomodoro record: $e, pomodoro_id=${event.pomodoroId}');
    }
  }
      ...
      if (event.remainingTime <= 0) {
        ...
        try {
          ...
          final rowsAffected = await db.update(
              ...
              whereArgs: [currentState.session.pomodoroId]);
          if (rowsAffected == 0) {
            print('WARNING: pomodoro completion update affected 0 rows, pomodoro_id=${currentState.session.pomodoroId}');
          }
        } catch (e) {
          print('CRITICAL: Error recording pomodoro completion: $e, pomodoro_id=${currentState.session.pomodoroId}');
        }
