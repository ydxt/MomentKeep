...
class DiaryBloc extends Bloc<DiaryEvent, DiaryState> {
  ...
  FutureOr<void> _onAddDiaryEntry(
    ...
    try {
      ...
      final pointsPerDiary = await UserSettingsService().getSettingInt(StorageKeys.pointsPerDiary, defaultValue: 5);
      
      // 添加日记完成积分
      final ruleSnapshot = jsonEncode({'points_per_diary': pointsPerDiary});
      ...
      if (e is SocketException || e is http.ClientException) {
        ...
        errorMessage = '添加智能日记失败: $e';
      }
      emit(DiaryError(errorMessage));
    }
  }
  ...
  FutureOr<void> _onDeleteDiaryEntry(
    ...
    try {
      ...
      if (entryIndex != -1) {
        ...
        final originalBillItem = await _databaseService.getBillItemByRelatedId(userId, deletedEntry.id, 'diary_completed');
        int pointsToDeduct = pointsPerDiary;
        if (originalBillItem?.ruleSnapshot != null) {
          try {
            final snapshot = jsonDecode(originalBillItem!.ruleSnapshot!);
            pointsToDeduct = snapshot['points_per_diary'] ?? pointsPerDiary;
          } catch (_) {}
        }
        await _databaseService.updateUserPoints(
          userId,
          -pointsToDeduct.toDouble(),
          description: '删除日记: ${deletedEntry.title}',
          transactionType: 'diary_deleted',
          relatedId: deletedEntry.id,
        );
