...
class AutoCleanupService {
  ...
  Future<int> getCleanupDays() async {
    ...
    return prefs.getInt(StorageKeys.cleanupDays) ?? _cleanupDays;
  }

  /// 设置清理时间配置（天数）
  Future<void> setCleanupDays(int days) async {
    final prefs = await SharedPreferences.getInstance();
    await prefs.setInt(StorageKeys.cleanupDays, days);
  }

  /// 获取最后清理时间
  Future<int?> getLastCleanupTime() async {
    final prefs = await SharedPreferences.getInstance();
    return prefs.getInt(StorageKeys.lastCleanupTime);
  }

  /// 设置最后清理时间
  Future<void> setLastCleanupTime(int time) async {
    final prefs = await SharedPreferences.getInstance();
    await prefs.setInt(StorageKeys.lastCleanupTime, time);
  }
  ...
  Future<void> cleanupExpiredProducts() async {
    try {
      ...
      // 获取所有软删除的商品
      final productMaps = await productDb.getAllProducts();
      final allProducts = productMaps.map((map) => StarProduct.fromMap(map)).toList();
