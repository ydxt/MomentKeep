...
class HybridAuthService {
  ...
  /// 保存本地用户会话
  Future<void> _saveLocalSession(String userId, String email, String username) async {
    final prefs = await SharedPreferences.getInstance();
    await prefs.setString(StorageKeys.userId, userId);
    await prefs.setString(StorageKeys.userEmail, email);
    await prefs.setString(StorageKeys.userUsername, username);
  }
  ...
  Future<AuthResult> cloudLogin({
    ...
    try {
      ...
      // 保存会话信息到本地（缓存）
      final prefs = await SharedPreferences.getInstance();
      await prefs.setString(StorageKeys.userId, response.user!.id);
      await prefs.setString(StorageKeys.userEmail, response.user!.email ?? '');
      await prefs.setString(StorageKeys.userUsername, response.user!.userMetadata?['username'] ?? '');
