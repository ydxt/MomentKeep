...
void main() async {
  ...
  await UserSettingsService().migrateFromSharedPreferences();
  ...
  Future.delayed(Duration.zero, () async {
    ...
    final hasRecharged = prefs.getBool(StorageKeys.testAccountRecharged) ?? false;
    final hasAddedTestData = prefs.getBool(StorageKeys.testDataAdded) ?? false;
    
    if (!hasRecharged) {
      try {
        final databaseService = DatabaseService();
        await databaseService.rechargePointsByEmail('test@h.com', 10000);
        await prefs.setBool(StorageKeys.testAccountRecharged, true);
    ...
    if (!hasAddedTestData) {
      try {
        ...
        if (user != null) {
          ...
          await prefs.setBool(StorageKeys.testDataAdded, true);
