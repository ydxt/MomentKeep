...
extension HabitViewTypeExtension on HabitViewType {
  ...
  String get displayName {
    switch (this) {
      ...
      case ScoringMode.weekly:
        return '按周计分';
      case ScoringMode.weeklyWithPenalty:
        return '按周计分2';
      case ScoringMode.custom:
        return '自定义周期';
      case ScoringMode.checkInWithPenalty:
        return '自定义周期2';
    }
  }
}
...
class Habit extends Equatable {
  ...
  final int minCheckInDays;            // 打卡+扣分模式：周期内最小打卡天数
  final int checkInPoints;             // 打卡+扣分模式：每次打卡得分
  final int penaltyPoints;             // 打卡+扣分模式：不足最小天数每次扣分
  ...
  const Habit({
    ...
    this.minCheckInDays = 0,
    this.checkInPoints = 0,
  ...
  Habit copyWith({
    ...
    return Habit(
      ...
      minCheckInDays: minCheckInDays ?? this.minCheckInDays,
  ...
  factory Habit.fromJson(Map<String, dynamic> json) {
    ...
    return Habit(
      ...
      lastCycleRewardTime: json['lastCycleRewardTime'] != null
          ...
          : null,
      minCheckInDays: json['minCheckInDays'] ?? 0,
      checkInPoints: json['checkInPoints'] ?? 0,
